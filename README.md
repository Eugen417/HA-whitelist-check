...
* **Информативные статусы на дашборде:** Датчик не просто показывает «Подключено / Отключено». При сбое прямо на карточке выводится точная причина: `HTTP 403 (Blocked)`, `HTTP 501`, `Timeout` или `PING Failed`. Идеально для отладки блокировок Cloudflare!
...
* **Умные проверки (Smart Fallback):** Интеграция поддерживает как веб-запросы (GET, POST, HEAD, PUT), так и системный `PING` (ICMP). Если IoT-сервер сбрасывает HTTP-запрос (как это делает Tuya или LinkLink), движок автоматически переключается на пинг для проверки физической доступности узла.
...
* **Обход защиты от ботов:** Имитация реального браузера (User-Agent) при HTTP-проверках снижает шанс ложного бана от Cloudflare и других систем защиты.
