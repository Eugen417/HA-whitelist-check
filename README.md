...
* **Умные проверки (Smart Fallback):** Интеграция умеет проверять узлы по HTTP (GET, POST, HEAD, PUT, PING). Если IoT-сервер сбрасывает HTTP-запрос (как это делает Tuya или LinkLink) или отвечает нестандартной ошибкой API, движок автоматически всё анализирует или переключается на системный `PING` (ICMP).
