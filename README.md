...
* **Умные проверки (Smart Fallback):** Интеграция умеет проверять узлы по HTTP (GET, POST, HEAD, PUT). Если IoT-сервер сбрасывает HTTP-запрос (как это делает Tuya или LinkLink), движок автоматически переключается на проверку открытых портов или системный `PING` (ICMP).
